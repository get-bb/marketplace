...
- **Self-healing setup.** The plugin registers Droid through bb's ACP providers plugin, so `setup` takes effect immediately with no bb restart. On every bb start it verifies the entry against your Droid install and repairs moved binaries or stale fields automatically.
